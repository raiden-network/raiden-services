...
def test_regression_693(pathfinding_service_mock):
    ...
    tn3 = make_token_network_address()
    ...
    network_event3 = ReceiveTokenNetworkCreatedEvent(
        token_address=to_canonical_address("0xf9BA8aDF7F7024D7de8eB37b4c981CFFe3C88Ea1"),
        token_network_address=tn3,
        block_number=BlockNumber(11688681),
    )
    pathfinding_service_mock.handle_event(network_event)
    pathfinding_service_mock.handle_event(network_event2)
    pathfinding_service_mock.handle_event(network_event3)
    ...
    pathfinding_service_mock.handle_event(channel_event)
    assert len(pathfinding_service_mock.token_networks) == 4
    ...
    assert len(pathfinding_service_mock.token_networks[tn3].channel_id_to_addresses) == 0

    assert database.get_channel(token_network_address=tn1, channel_id=id1) is not None
    assert database.get_channel(token_network_address=tn2, channel_id=id1) is None
    assert database.get_channel(token_network_address=tn3, channel_id=id1) is None
    ...
    assert database.get_channel(token_network_address=tn1, channel_id=id1) is not None
    assert database.get_channel(token_network_address=tn2, channel_id=id1) is not None
    assert database.get_channel(token_network_address=tn3, channel_id=id1) is None

    channel_event3 = ReceiveChannelOpenedEvent(
        token_network_address=tn3,
        channel_identifier=ChannelID(1),
        participant1=p1,
        participant2=p2,
    ...
    pathfinding_service_mock.handle_event(channel_event3)
    assert len(pathfinding_service_mock.token_networks) == 4
    assert len(pathfinding_service_mock.token_networks[tn1].channel_id_to_addresses) == 1
    assert len(pathfinding_service_mock.token_networks[tn2].channel_id_to_addresses) == 1
    assert len(pathfinding_service_mock.token_networks[tn3].channel_id_to_addresses) == 1

    assert database.get_channel(token_network_address=tn1, channel_id=id1) is not None
    assert database.get_channel(token_network_address=tn2, channel_id=id1) is not None
    assert database.get_channel(token_network_address=tn3, channel_id=id1) is not None
