...
def test_pfs_rejects_capacity_update_with_wrong_channel_identifier(
    ...
    assert token_network.channel_id_to_addresses[ChannelID(0)] == (addresses[0], addresses[1])
...
def test_pfs_rejects_capacity_update_with_impossible_updating_capacity(
    ...
    token_network.handle_channel_new_deposit_event(
        channel_identifier=ChannelID(0), receiver=addresses[0], total_deposit=100
    )

    token_network.handle_channel_new_deposit_event(
        channel_identifier=ChannelID(0), receiver=addresses[1], total_deposit=100
    )
...
def test_pfs_rejects_capacity_update_with_wrong_updating_participant(
    ...
    message = get_updatepfs_message(
        updating_participant=addresses[2], other_participant=addresses[1]
    )
...
def test_pfs_rejects_capacity_update_with_wrong_other_participant(
    ...
    message = get_updatepfs_message(
        updating_participant=addresses[0], other_participant=addresses[2]
    )
...
def test_pfs_rejects_capacity_update_with_wrong_nonces(
    ...
    message = get_updatepfs_message(
        updating_participant=addresses[0], other_participant=addresses[1]
    )
