...
def test_register_mints_tokens_and_deposits(web3, service_registry, get_accounts):
    account = decode_hex(get_accounts(1)[0])

    deposit_to_registry(
        web3=web3, service_registry_contract=service_registry, service_address=Address(account),
    )
...
def test_update_url_set_new_url(web3, service_registry, get_accounts):
    account = decode_hex(get_accounts(1)[0])
